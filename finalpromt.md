...
- **NEW:** Options can contain commas and hyphens (e.g., "0 - all are used, 1-2 ads")
- **Parsing improved:** Correctly splits "0 - none, 1-2 people, 3-4 items, No answer" format
- Safe to use complex numbered options or ranges in your choices
...
**Two modes:**
1. **Standard Matching:** Different categories → drag-and-drop interface
2. **Interactive Categorization:** All items same category → user clicks each item to mark it
...
- **UI automatically switches mode:** If all left items are the same, shows interactive categorization instead of drag-and-drop
- **Interactive mode:** User must click/check ALL items before submitting (visual progress tracking)
- **Visual legend:** Shows drag-drop instructions or category markers clearly
...
**When to use:** Test understanding of passages, matching ads/persons to criteria, TRUE/FALSE/NOT MENTIONED questions

**Format 1: Matching Ads (Search Criteria + Ads)**
```json
{
  "type": "reading",
  "context": "Person A sucht: Wohnung in München, 2 Zimmer, mit Balkon, maximal 800€ | Ad 1: 2-Zimmer-Wohnung in München, mit Balkon, 750€ | Ad 2: 3-Zimmer-Wohnung in München, 900€ | Ad 3: 2-Zimmer-Wohnung in Berlin, mit Balkon, 700€",
  "text": "Which ad matches Person A?",
  "answer": "Ad 1"
}
```
**Important:** Items with "sucht:" are search criteria (not selectable). Only ads are clickable options.
...
{
  "type": "reading",
  "context": "Person A: 'Ich bin dafür, dass man mehr Rad fährt. Das ist gesund.' | Person B: 'Ich bin dagegen. Bei schlechtem Wetter ist das Auto besser.' | Person C: 'Radfahren ist OK, aber nicht im Winter.'",
  "text": "Statement: 'Radfahren hat Vorteile für Gesundheit.' Who said this?",
  "answer": "Person A"
}
```
**Important:** ALL persons (A, B, C, D, etc.) are clickable options. No person is "search criteria" in this format.

**Key points:**
- **For matching ads (Format 1):** `context` = "Person X sucht: criteria | Ad 1: ... | Ad 2: ..."
  - Items with "sucht:" are NOT clickable (they're search criteria)
  - Only ads/options after the criteria are clickable
- **For person identification (Format 3):** `context` = "Person A: statement | Person B: statement | ..."
  - ALL persons are clickable options (no "sucht:" here!)
  - Each person's label becomes a selectable button
- **For TRUE/FALSE (Format 2):** `context` = "Text: '[passage]'" (single passage, no pipes)
  - UI automatically generates TRUE/FALSE/NOT MENTIONED buttons
  - Answer must be exactly: "TRUE", "FALSE", or "NOT MENTIONED"
- `text` = the question or statement to evaluate
- `answer` = the label only (e.g., "Ad 1", "Person A", "TRUE")
- UI shows passage + statement clearly separated
- User clicks directly on the option (no separate input field)
...
**When to use:** Case identification, grammar analysis, creating checklists, extracting information

**IMPORTANT:** This type is flexible for ANY subject - not just reading! Use for grammar, vocabulary, sentence building, etc.

```json
{
  "type": "identify",
  "text": "Person D sucht: Fahrrad, gebraucht, maximal 150€, mit Licht",
  "context": "Create a checklist for Person D (4 requirements)",
  "answer": "1. Fahrrad | 2. gebraucht | 3. ≤150€ | 4. mit Licht"
}
```

**Alternative - Grammar analysis:**
```json
{
  "type": "identify",
...
**Key points:**
- `text` = source material to analyze
- `context` = task description (what to identify/list)
- `answer` = use pipe `|` to separate items
- Accepts multiple formats: line-by-line, comma-separated, numbered with pipes
- UI shows large textarea with format examples
- Works for: reading comprehension, grammar analysis, vocabulary categorization, checklist creation
...
**Key points:**
- `text` = **Scenario description** (displayed as prominent card at top with 🎭 emoji)
...
- Each turn can have **multiple {blank} markers**
- If multiple blanks in one turn, separate answers with commas: `"mir, meinem Bruder"`
- **Immediate feedback:** After each turn, user sees green/red coloring instantly
- Conversation reveals turn-by-turn progressively
- Alternating speakers create natural dialogue flow
- Chat-bubble style UI with color-coded speakers
